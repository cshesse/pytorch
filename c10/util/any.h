...
// so that it can also be used in other backports such as `c10::optional` and `c10::variant`.

// yf225 TODO: explain why the following is removed:
// template< class T >
...
//     return in_place_t();
// }
// yf225 TODO: add tests for c10::any in `aten/src/ATen/test/variant_test.cpp`
// yf225 TODO: revert changes to Variable hooks, we should have those changes in the module hooks PR instead to show why we need those changes
...
#define any_REQUIRES_T(...) \
    , typename = typename std::enable_if< (__VA_ARGS__), c10::any_lite::detail::enabler >::type
...
//  any_REQUIRES_T(...) Allow for VC120 (VS2013):
    , typename = typename std::enable_if< (std::is_reference<ValueType>::value || std::is_copy_constructible<ValueType>::value), c10::any_lite::detail::enabler >::type
#endif
>
